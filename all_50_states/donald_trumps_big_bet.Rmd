---
title: "Donald Trump's Big Bet on Less Educated - NY Times"
...
The goal is to reproduce one of the best visualizations on the 2016 US presidential
election that correctly predicted how the demographics within each county could be
the key in determining the election outcome.

Leveraging the power of the `tidyverse` and `sf` packages, I replicated the map that 
closely matches the original plot.
...
and effort was dedicated on rummaging through the internet for the sources
and verifying the metrics in each step of the calculation.
...
path_cps_nov <- "C:/Users/logic/Downloads/election_files/cps_00003_1.csv"

# Voting age population data broken down by race
path_voting_age <- "C:/Users/logic/Downloads/election_files/County.csv"

# White (not hispanic) population over 25 divided by educational attainment
path_white_edu <- "C:/Users/logic/Downloads/election_files/ACS_15_5YR_C15002H_with_ann.csv"

# Geodata
path_county_geo <- "C:/Users/logic/Downloads/election_files/cb_2015_us_county_20m_sf.rds" 
path_state_geo <- "C:/Users/logic/Downloads/election_files/cb_2015_us_state_20m_sf.rds"
```
...
*Alaska and Hawaii are repositioned to be displayed by the projection used by other 48 states.
Credit goes to Claus Wilke whose code was slightly modified to accomplish this task [^2].
...
[^2]: Claus Wilke's code for repositioning / rescaling Alaska & hawaii [US_income.R](https://github.com/clauswilke/dviz.supp/blob/master/data-raw/tidy_census/US_income.R)